...
function createBaseResponse(): Response {
  return { id: 0, pricer: "", request: "", price: "", creator: "" };
}
...
function createBaseEventCreateResponse(): EventCreateResponse {
  return { id: 0 };
}

export const EventCreateResponse = {
  encode(message: EventCreateResponse, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== 0) {
      writer.uint32(8).uint64(message.id);
    }
    return writer;
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): EventCreateResponse {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseEventCreateResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromPartial<I extends Exact<DeepPartial<EventCreateResponse>, I>>(object: I): EventCreateResponse {
    const message = createBaseEventCreateResponse();
    message.id = object.id ?? 0;
    return message;
  },
